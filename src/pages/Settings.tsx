...
import { Settings as SettingsType } from "@/types/session";
import { toast } from "sonner";

export default function Settings() {
  const { settings, updateSetting } = useSettings();

  const focusAreaOptions: { value: SettingsType["focusArea"]; label: string }[] = [
  ...
  const handleFocusChange = (value: SettingsType["focusArea"]) => {
    updateSetting("focusArea", value);
    toast.success("Focus area updated");
  };

  const handleMotivationToggle = (value: boolean) => {
        ...
        <div className="max-w-xl mx-auto space-y-6">
          ...
          <div className="bg-card rounded-2xl p-6 border border-border">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Set your target practice time for each day
            </p>
            <div className="flex items-center gap-4">
              ...
              <div className="w-20 text-center">
                ...
                <span className="text-muted-foreground text-sm ml-1">min</span>
              </div>
            </div>
          </div>
